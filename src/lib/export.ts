...
export async function downloadPdf({ meta, set, diagrams = {}, signature }: ExportArgs) {
  ...
  for (const slot of getPattern(meta.marks)) {
    ...
    const text = `${label} ${q?.text ?? ""}   [${slot.marks} marks | ${q?.co ?? ""} | ${q?.bloom ?? slot.bloom}]`;
...
export async function downloadWord({ meta, set, signature }: ExportArgs) {
  ...
  const rows = [
    new TableRow({
      children: ["Q. No.", "Sub Q.", "Statement of Question", "Marks", "CO", "BT Level"].map(
    ...
    ...getPattern(meta.marks).map((slot) => {
      const q = set.questions.find((x) => x.key === slot.key);
      return new TableRow({
        children: [slot.qNo, slot.subQ, q?.text ?? "", String(slot.marks), q?.co ?? "", q?.bloom ?? slot.bloom].map(
